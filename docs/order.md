...
- **Order placement reads the catalog directly.** `OrderRepository` reads `product`/`productVariant`/`comboProduct` with its own lean `select`s (`findProductForOrder`/`findVariantForOrder`/`findComboForOrder`), the same pattern `InventoryRepository` already uses for its own "stock target" lookups (`findProductStockInfo`/`findVariantStockInfo`) — it does not go through `ProductService`/`ComboProductService`. `OrderModule` does import `ComboProductModule`, but only for `ComboProductRepository.adjustSoldQuantity`: moving a combo's sold counter is a write against a column with two triggers hanging off it, and that belongs to the module that owns the column rather than being open-coded here.
...
- **A combo purchase decrements the underlying products/variants, never `ComboProduct.availableQuantity` directly** — that column is trigger-derived and recomputes itself once the underlying stock changes, per `combo-product.prisma`'s own documented contract. The one combo column placement *does* write is `soldQuantity`, which nothing else can derive: the combo holds no stock, so component movements alone cannot record that a bundle was sold.
...
3. **Per-line validation + snapshot build** (`buildOrderItems`):
   ...
   - **Combo**: must be `ACTIVE`, not soft-deleted, and **inside its `startsAt`/`endsAt` window** — the window is tested live here rather than trusted to `status`, because `status` is only corrected by an hourly sweep and honouring an expired promotional price is a real loss (see `docs/combo-product.md` → **Offer Lifecycle**). Effective sellable bundles = `offeredQuantity IS NULL ? availableQuantity : min(availableQuantity, max(offeredQuantity - soldQuantity, 0))` — the same formula the SQL function `combo_sellable_quantity` and `ComboProductService.computeSellableQuantity` use; **all three must agree.** A combo line also expands into a stock-deduction entry per underlying `ComboItem` (`ComboItem.quantity × bundles purchased`), merged with any other line touching the same product/variant.
   - A zero here has two different cures, so it carries two different messages: *"cannot be assembled right now — out of stock"* (restocking fixes it) versus *"sold out — every offered bundle has been claimed"* (only raising the cap does).
...
11. **`ComboProduct.soldQuantity` incremented** for every combo line (`applyComboSoldQuantity`), in this same transaction. Step 10 touches a combo's *components*, which cannot record that a **bundle** was sold — and a bundle is what consumes a capped offer. Writing the column also fires `trg_sync_combo_offer_exhaustion`, so a combo that just sold its last offered bundle goes `INACTIVE` before this transaction commits. See `docs/combo-product.md` → **Offer Lifecycle**.
12. **Initial `OrderStatusHistory` row created** (`status: PENDING`, `note: 'Order placed'`, `changedBy: null` — customer/system-initiated).
13. **`Payment` row created** (`type: CHARGE`, `status: PENDING`, `amount: totalAmount`).
14. Order re-fetched inside the same transaction and returned as `OrderResponseDto`.
...
**The same transitions also release combo bundles.** `releaseComboSoldQuantity` decrements `ComboProduct.soldQuantity` by what this order claimed, reading the combo lines back from `order_items` (grouped per combo — one order can legitimately carry the same combo on more than one line). Restoring component stock *without* this would leave a capped offer permanently consumed by a sale that was undone: the bundles would be back on the shelf while the combo stayed retired. The decrement floors at zero in SQL (`GREATEST(sold_quantity + delta, 0)`), so a double-restore cannot trip `combo_products_sold_quantity_non_negative` from inside a path that has already handed the stock back.
